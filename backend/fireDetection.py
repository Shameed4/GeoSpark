...
def generate_wildfire_report(video_source, coordinates, json_file_path="wildfire_report.json"):
    ...
    # Create the new report dictionary
    report = {
        "fire": fire_detected,
    ...
    # Create a key for the report using the coordinates.
    report_key = f"{lat},{lon}"

    # Load existing reports from the file as a dictionary.
    reports = {}
    ...
    # Add or update the report for these coordinates.
    reports[report_key] = report

    # Write the updated dictionary back to the file.
    with open(json_file_path, "w") as f:
        json.dump(reports, f, indent=4)
    print(f"Wildfire report saved with key '{report_key}' to {json_file_path}")
